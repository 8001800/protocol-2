...
contract("IdentityProvider", accounts => {
  ...
  it("should update the identity if request exists", async () => {
    ...
    assert.equal(
      requestServiceLogs[0].args.providerId.toNumber(),
      params.providerId
    );
    assert.equal(
      requestServiceLogs[0].args.providerVersion.toNumber(),
    ...
    assert.equal(
      requestServiceLogs[0].args.requestId.toNumber(),
      params.requestId
    );
    ...
    assert.equal(
      acceptServiceLogs[0].args.providerId.toNumber(),
      params.providerId
    );
    assert.equal(acceptServiceLogs[0].args.requester, params.requester);
    assert.equal(
      acceptServiceLogs[0].args.requestId.toNumber(),
      params.requestId
    );
    ...
    assert.equal(
      completeServiceLogs[0].args.providerId.toNumber(),
      params.providerId
    );
    assert.equal(completeServiceLogs[0].args.requester, params.requester);
    assert.equal(
      completeServiceLogs[0].args.requestId.toNumber(),
      params.requestId
    );
  ...
  it("should write attestations after update", async () => {
    ...
    const completeServiceEvents = await promisify(cb =>
      kernel
        .ServicePerformed(
          {},
          { fromBlock: completeService.receipt.blockNumber, toBlock: "latest" }
        )
        .get(cb)
    )();
    ...
    assert.equal(
      completeServiceEvents[0].args.providerId.toNumber(),
      params.providerId
    );
    assert.equal(completeServiceEvents[0].args.requester, params.requester);
    assert.equal(
      completeServiceEvents[0].args.requestId.toNumber(),
      params.requestId
    );
